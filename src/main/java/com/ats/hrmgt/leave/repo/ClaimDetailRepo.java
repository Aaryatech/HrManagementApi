...
public interface ClaimDetailRepo extends JpaRepository<ClaimDetail, Integer>{
	...
	@Query(value = " SELECT\n" + 
			...
			"d.emp_dept_name,\n" + 
			"ct.claim_type_title,"+ 
			"p.project_title as project_type_title \n" + 
			"FROM\n" + 
			"claim_apply AS ca,\n" + 
			"emp_info AS e,\n" + 
			"m_emp_department d,\n" + 
	...
	@Query(value = " SELECT\n" + 
			...
			+ "project_type as p\n" + 
			"WHERE \n" + 
			"ca.emp_id =:empId  AND ca.emp_id = e.emp_id AND ca.del_status = 1 AND d.emp_dept_id = e.emp_dept_id AND ca.claim_type_id = ct.claim_type_id AND ca.project_id=p.project_type_id ", nativeQuery = true)
	List<ClaimDetail> getClaimList(@Param("empId") int empId);
