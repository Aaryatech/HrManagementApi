...
public interface HolidayRepo extends JpaRepository<Holiday, Integer> {
	...
	@Query(value="select w.* from m_holiday w,emp_info e where  FIND_IN_SET(e.loc_id=w.loc_id,w.loc_id)  and e.emp_id=:empId and w.del_status=1 and w.cal_yr_id=(select cal_yr_id from dm_cal_year where is_current=1) "
