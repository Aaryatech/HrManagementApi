...
public interface GetHolidayRepo extends JpaRepository<GetHoliday, Integer> {
	...
	@Query(value = "  SELECT h.holiday_id,h.holiday_fromdt,h.holiday_todt,h.holiday_remark,h.ex_var1,"
			...
			+ " h.company_id=:companyId  AND  h.loc_id=:locId  AND h.del_status=1 AND h.is_active=1 AND h.company_id=h.company_id AND"
			+ " l.loc_id=h.loc_id AND cy.cal_yr_id=h.cal_yr_id AND c.del_status=1 AND l.del_status=1 ", nativeQuery = true)

	List<GetHoliday> getHolidayListByCompany(@Param("companyId") int companyId, @Param("locId") int locId);
